...
const findGrapeTwo =function (arr) { //=> function written as expression
    if(arr[i].color === "purple") { //=> checking if arr position is equal to purple
        return console.log(`The fruit with index ${arr.indexOf(arr[i])} is a grape`) //=> if condition met, then printing
    } 
}

// RE-WRITE THE ABOVE FUNCTION IN 'ARROW FUNCTION' SYNTAX HERE.
const findGrapeThree = (arr) => { //=> function written as expression
    if(arr[i].color === "purple") {//=> checking if arr position is equal to purple
        return console.log(`The fruit with index ${arr.indexOf(arr[i])} is a grape`)//=> if condition met, then printing
    } 
}
...
// CODE HERE
// => writting function using arrow method, returning line
const someFunction = (pam1, pam2) => {return `The first parameter is ${pam1}. The second parameter is ${pam2}.`}

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
console.log(someFunction('first-parameter', 'second-parameter')) // => printing
...
function pricesPlusTax(array, callback){
    // for(price of prices){totalCost.push(price*1.20)} //=> alternate way to write this
    for(i=0; i<array.length; i++){  // => looping through array
        totalCost.push(array[i]*1.20) // => pushing the new value to empty array
    }
    callback(totalCost) // => callback function passing 'totalCost'
}
...
// CODE HERE
function multiplyingFactory(paramOne){
    return (paramtwo) =>{ paramOne>=5 ? console.log(paramOne*paramtwo) : console.log("Cannot multiply: the first number is smaller than 5.") }
}
...
timesFour(4)
// multiplyingFactory(6)(5)  // =>testing code out for self only...not part of answer
...
let timesFourTwo = multiplyingFactory(5)
