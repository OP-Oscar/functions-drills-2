...
// CODE HERE
function multiply(numbOne, numTwo, cb){ // =>function with two paramters and a callback
  cb(numbOne * numTwo); // => callback function does not need to be explicitely defined
}
...
// CODE HERE 
function first(array, cb){ // =>declaring a function with one parameter and callback
  cb(array[0]) // =>callback function with the arrays first element 
}
...
first(names, firstName => {
  console.log('The first name in names is ' + firstName)  // =>First name should be Tyler
})
...
// CODE HERE
const last = function (array, cb){  // => creating a function expression called last 
  cb(array[array.length-1])                     // => callback function with last element in array
}
...
last(names, lastName => { 
  console.log('The last name in names is ' + lastName)  // => should return cahlan
})
...
// CODE HERE 
const contains = (array, name, callback) => {  // = creating function using arrow
  if(array.includes(name)){  // =check if array includes names value
    callback(true)   // =>callback function
  }
  else{
...
// create function that takes 2 parameters an array and a callback func
function uniq(array,pizza){
  ...
  //invoke callback with modified array
  pizza(cleanArray)
}

// // creating shorter version of function that takes 2 parameters an array and a callback func
// function uniq(array,pizza){
// //remove duplicates in array, using set
// //invoke callback with modified array
// pizza(Array.from(new Set(array)))
// }
...
// CODE HERE
function addingFactory(paramOne){
  return (paraminside) => paramOne+paraminside
}
//shorter way to write the code above
// const addingFactory = (paramOne) => (paramTwo) => paramTwo + y;
...
// CODE HERE
let addTen = addingFactory(10) // =>creating new variable and invoking addingFactory
